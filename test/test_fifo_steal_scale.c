...
void* run_function(void* param) {
  ...
  for (i = 0; i < PER_THREAD_COUNT; ++i) {
    ...
    if (action < 4) {
      ...
      n->data = (void*)i;
      const int ret = fifo_steal_buffer_push(my_fifo, n);
      assert(ret);
      ++my_data->push_count;
    } else {
      ...
      int stole = 0;
      while (tries > 0) {
        fifo_steal_buffer_t* const steal_fifo = fifo[j % NUM_THREADS];
        node_t* n = NULL;
        int ret;
        do {
...
int main(int argc, char* argv[]) {
  ...
  for (i = 0; i < NUM_THREADS; ++i) {
    void* to_free = NULL;
    while (fifo_steal_buffer_pop(fifo[i], &to_free)) {
      free(to_free);
    }
    ...
    printf(
        ...
        "%lld\n",
        (int)i, data[i].push_count, data[i].pop_count, data[i].steal_count,
        data[i].attempt_count, data[i].empty_count);
  ...
  printf("timing: %d threads %d events %d work %lf seconds\n", NUM_THREADS,
         PER_THREAD_COUNT, WORK_FACTOR, seconds);
