...
void* run_function(void* param) {
  ...
  hazard_pointer_thread_record_t* my_record =
      hazard_pointer_thread_record_create_and_push(&head, POINTERS_PER_THREAD);
  ...
  // need to finish cleaning records owned by this thread. M.M. Michael's paper
  // suggests methods to let threads leave...not bothering
...
int main() {
  ...
  const size_t BUFFER_SIZE =
      ...
      POINTERS_PER_THREAD;  // each thread can have up to 2 * N * K, so we need
                            // up to N * 2 * N * K nodes available at any given
                            // time
