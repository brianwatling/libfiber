...
class UnboundedMultiProducerChannel {
 ...
 public:
  ...
  struct event {
   ...
   public:
    ...
    const event_descriptor& getType() const { return type; }

    event_payload& getPayload() { return payload; }

    const event_payload& getPayload() const { return payload; }

    void setPayload(const event_payload& p) { payload = p; }
  ...
  UnboundedMultiProducerChannel(fiber_signal_t* signal) {
    if (!fiber_unbounded_channel_init(&channel, signal)) {
      throw std::runtime_error(
          "fiberpp::UnboundedMultiProducerEventSource() - error creating "
          "channel");
    }
  }
  ...
  event* receive() {
    event_wrapper* const wrapper = reinterpret_cast<event_wrapper*>(
 ...
 private:
  struct event_wrapper : public fiber_unbounded_channel_message_t {
    event_wrapper(const event_descriptor& type) : e(type, this) {
      this->data = this;
    }
 ...
 public:
  static event* allocEvent(const event_descriptor& type) {
    event_wrapper* const wrapper =
        reinterpret_cast<event_wrapper*>(malloc(sizeof(*wrapper)));
    if (!wrapper) {
      throw std::bad_alloc();
    }
    try {
...
class UnboundedSingleProducerChannel {
 ...
 public:
  ...
  event* receive() {
    event_wrapper* const wrapper = reinterpret_cast<event_wrapper*>(
        fiber_unbounded_sp_channel_receive(&channel));
    event* const e = &reinterpret_cast<event_wrapper*>(wrapper->data)->e;
    e->wrapper = wrapper;
    return e;
  }
  ...
  event* tryReceive() {
    event_wrapper* const wrapper = reinterpret_cast<event_wrapper*>(
        fiber_unbounded_sp_channel_try_receive(&channel));
    if (!wrapper) {
      return NULL;
    }
    event* const e = &reinterpret_cast<event_wrapper*>(wrapper->data)->e;
...
class ChannelSelector {
 public:
  ...
  ChannelSelector(size_t numChannels, ChannelType** channels,
                  fiber_signal_t* signal)
      : signal(signal), numChannels(numChannels), channels(channels) {
    if (!numChannels || !channels) {
      throw std::runtime_error(
          "fiberpp::ChannelSelector() - must provide channels");
    }
    for (size_t i = 0; i < numChannels; ++i) {
      if (!channels[i]) {
        throw std::runtime_error(
            "fiberpp::ChannelSelector() - NULL channel provided");
      }
    }
  }
  ...
  static void freeEvent(event* e) { ChannelType::freeEvent(e); }
