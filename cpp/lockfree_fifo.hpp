...
yield
sleep
condition variable?? (need to make sure the fifo is still empty after acquiring
the lock)

** each wait strategy should have a next_strategy which is used after attempting
the first strategy
*/

template <typename T, typename ProducerTraits, typename ConsumerTraits,
          typename Allocator>
struct fifo_impl {};
...
struct fifo_node {
  ...
  fifo_node(T const& data) : next(nullptr), data(data) {}

  fifo_node(T&& data) : next(nullptr), data(std::move(data)) {}
...
struct push_impl<fifo_traits<concurrency::single, progress_type::wait_free>,
                 NodeType> {
...
template <typename NodeType>
struct push_impl<fifo_traits<concurrency::multiple, progress_type::wait_free>,
                 NodeType> {
  typedef typename NodeType::value_type value_type;
  typedef typename NodeType::pointer node_pointer;
  ...
  bool try_push(node_pointer newNode) {
    node_pointer const oldTail =
        tail.exchange(newNode, std::memory_order_seq_cst);
    oldTail->next.store(newNode, std::memory_order_release);
    return true;
  }
...
template <typename NodeType>
struct pop_impl<fifo_traits<concurrency::single, progress_type::wait_free>,
                NodeType> {
  typedef typename NodeType::value_type value_type;
  typedef typename NodeType::pointer node_pointer;
  ...
  PopResult try_pop(value_type& dest) {
    ...
    node_pointer const oldHeadNext = reinterpret_cast<node_pointer>(
...
struct tagged_pointer_snapshot {
 ...
 private:
  tagged_pointer_snapshot(TagType t, T v) : t(t), v(v) {}
...
struct tagged_pointer {
  ...
  tagged_pointer() : value(), tag(0) {}

  tagged_pointer(T value) : value(value), tag(0) {}
  ...
  bool compare_and_swap(snapshot_type const& original, T updated) {
    ...
    __asm__ __volatile__(
        ...
        "setz %0"
        : "=q"(result), "+m"(tag)
        : "d"(original.v), "a"(original.t), "c"(updated), "b"(original.t + 1)
        : "cc");
    return result;
#else
#error please define a compare_and_swap2()
 ...
 private:
  tagged_pointer(TagType tag, T value) : tag(tag), value(value) {}
...
template <typename NodeType>
struct pop_impl<fifo_traits<concurrency::multiple, progress_type::lock_free>,
                NodeType> {
  typedef typename NodeType::value_type value_type;
  typedef typename NodeType::pointer node_pointer;
  ...
  PopResult try_pop(value_type& dest) {
    ...
    if (oldHeadNext) {
      static_assert(std::is_pod<value_type>::value,
                    "must use POD type, since pop may access freed memory");
...
struct sleep {
  void wait() { std::this_thread::sleep_for(DurationType(count)); }
};

}  // namespace wait_strategy

template <
    typename T, typename ProducerTraits, typename ConsumerTraits,
    typename BackoffStrategy = wait_strategy::spin<128, wait_strategy::relax>,
    typename PopWaitStrategy = wait_strategy::spin<128, wait_strategy::relax>,
    typename Allocator = std::allocator<T>, size_t CacheSize = 64>
class fifo {
 ...
 public:
  fifo() : pusher(allocator), popper(allocator, pusher.tail) {}
