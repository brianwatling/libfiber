...
/*
    Description: A multi-producer single-consumer FIFO based on a
   single-producer single-consumer FIFO . Each producer will have their own SPSC
                 FIFO. A single consumer acts as the consumer for all
                 producers. It's not required to have a thread per producer.
   This queue is wait-free - that is, any enqueue or dequeue operation is
   independent of other threads. NOTE: this MPSC FIFO provides *per-producer*
   FIFO and makes a best effort attempt to round-robin pops accross all
   producers.
...
static inline mpscr_fifo_t* mpscr_fifo_create(size_t num_producers) {
  ...
  mpscr_fifo_t* const ret =
      malloc(sizeof(*ret) + num_producers * sizeof(spsc_fifo_t));
  if (!ret) {
    return NULL;
  }
  ret->counter = 0;
...
static inline void mpscr_fifo_push(mpscr_fifo_t* f, size_t producer_number,
                                   spsc_node_t* new_node) {
  assert(f);
