...
static inline void fifo_steal_buffer_init(fifo_steal_buffer_t* fifo,
...
static inline fifo_steal_buffer_t* fifo_steal_buffer_create(
    uint32_t power_of_2_size) {
  assert(power_of_2_size && power_of_2_size < 32);
  const uint32_t size = 1 << power_of_2_size;
  const uint32_t required_size =
      sizeof(fifo_steal_buffer_t) + size * sizeof(void*);
...
// master thread only!
static inline int fifo_steal_buffer_push(fifo_steal_buffer_t* fifo,
                                         void* data) {
...
static inline int fifo_steal_buffer_steal(fifo_steal_buffer_t* fifo,
                                          void** out) {
  const uint64_t high = fifo->high;
  load_load_barrier();  // read high first; this means the buffer will appear
                        // smaller or equal to its actual size
...
static inline sharded_fifo_steal_buffer_t* sharded_fifo_steal_buffer_create(
    uint32_t num_shards, uint32_t shard_power_of_2_size) {
  const uint32_t shard_size = 1 << shard_power_of_2_size;
  const uint32_t memory_per_shard =
      sizeof(fifo_steal_buffer_t) + shard_size * sizeof(void*);
  const uint32_t total_memory =
      sizeof(sharded_fifo_steal_buffer_t) + num_shards * memory_per_shard;
...
static inline void sharded_fifo_steal_buffer_destroy(
    sharded_fifo_steal_buffer_t* fifo) {
  free(fifo);
}

static inline uint32_t sharded_fifo_steal_buffer_next_shard(
    sharded_fifo_steal_buffer_t* fifo, uint32_t* which) {
...
static inline fifo_steal_buffer_t* sharded_fifo_steal_buffer_get_shard(
    sharded_fifo_steal_buffer_t* fifo, uint32_t index) {
...
static inline int sharded_fifo_steal_buffer_push(
    sharded_fifo_steal_buffer_t* fifo, void* data) {
  const uint32_t num_shards = fifo->num_shards;
  uint32_t i;
  for (i = 0; i < num_shards; ++i) {
    const uint32_t shard =
        sharded_fifo_steal_buffer_next_shard(fifo, &fifo->next_shard_push);
    fifo_steal_buffer_t* const shard_to_push =
        sharded_fifo_steal_buffer_get_shard(fifo, shard);
...
static inline int sharded_fifo_steal_buffer_pop(
    sharded_fifo_steal_buffer_t* fifo, void** out) {
  const uint32_t num_shards = fifo->num_shards;
  uint32_t i;
  for (i = 0; i < num_shards; ++i) {
    const uint32_t shard =
        sharded_fifo_steal_buffer_next_shard(fifo, &fifo->next_shard_pop);
    fifo_steal_buffer_t* const shard_to_pop =
        sharded_fifo_steal_buffer_get_shard(fifo, shard);
...
static inline int sharded_fifo_steal_buffer_steal(
    sharded_fifo_steal_buffer_t* fifo, uint32_t hint, void** out) {
  const uint32_t num_shards = fifo->num_shards;
  uint32_t i = hint;
  const uint32_t end = hint + num_shards;
  int final_ret = SHARDED_FIFO_STEAL_BUFFER_EMPTY;
  for (; i < end; ++i) {
    fifo_steal_buffer_t* const shard_to_steal =
        sharded_fifo_steal_buffer_get_shard(fifo, i % num_shards);
    ...
    if (ret == FIFO_STEAL_BUFFER_SUCCESS) {
      ...
      // report aborted if *any* attempt aborts - if we get nothing this means
      // we may want to retry
      final_ret = ret == FIFO_STEAL_BUFFER_ABORT
                      ? SHARDED_FIFO_STEAL_BUFFER_ABORT
                      : final_ret;
    }
  }
  return final_ret;
