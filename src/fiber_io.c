...
ssize_t __xnet_sendto(int sockfd, const void* buf, size_t len, int flags,
...
typedef int (*acceptFnType)(int s, struct sockaddr* _RESTRICT_KYWD addr,
                            Psocklen_t addrlen);
#define ACCEPTPARAMS \
  int sockfd, struct sockaddr *_RESTRICT_KYWD addr, Psocklen_t addrlen

typedef ssize_t (*recvfromFnType)(int, void* _RESTRICT_KYWD, size_t, int,
                                  struct sockaddr* _RESTRICT_KYWD, Psocklen_t);
#define RECVFROMPARAMS                                         \
  int sockfd, void *_RESTRICT_KYWD buf, size_t len, int flags, \
      struct sockaddr *_RESTRICT_KYWD src_addr, Psocklen_t addrlen
...
typedef ssize_t (*recvfromFnType)(int, void*, size_t, int, struct sockaddr*,
                                  socklen_t*);
#define RECVFROMPARAMS                                                     \
...
typedef ssize_t (*sendtoFnType)(int sockfd, const void* buf, size_t len,
                                int flags, const struct sockaddr* dest_addr,
                                socklen_t addrlen);
typedef ssize_t (*sendmsgFnType)(int sockfd, const struct msghdr* msg,
                                 int flags);
...
static inline int should_block(int fd) {
  assert(fd >= 0);
  if (!thread_locked && fd_info && fd < max_fd &&
      fd_info[fd].flags_ & (IO_FLAG_BLOCKING | IO_FLAG_WAITABLE)) {
    return 1;
  }
  return 0;
}
...
ssize_t recv(int fd, void* buf, size_t len, int flags) {
  ...
  do {
    ...
    ret = fibershim_recv(fd, buf, len, flags);
           ...
           !(flags & MSG_DONTWAIT) && should_block(fd));
...
ssize_t recvfrom(RECVFROMPARAMS) {
  ...
  do {
    ...
    ret = fibershim_recvfrom(sockfd, buf, len, flags, src_addr, addrlen);
           ...
           !(flags & MSG_DONTWAIT) && should_block(sockfd));
...
int nanosleep(const struct timespec* rqtp, struct timespec* rmtp) {
  if (!thread_locked && fiber_manager_get()) {
    ...
    if (!fibershim_nanosleep) {
      fibershim_nanosleep = (nanosleepFnType)dlsym(RTLD_NEXT, "nanosleep");
    }
