...
int fiber_event_init() {
  ...
  write_barrier();  // needed so active_threads is set before fiber_loop (see
                    // fiber_poll_events_blocking - active_threads should never
                    // be decremented before it's been set)
...
size_t fiber_poll_events_blocking(uint32_t seconds, uint32_t useconds) {
  ...
  load_load_barrier();  // needed to ensure we read fiber_loop first - we need a
                        // valid active_thread count, so init writes
                        // active_threads then fiber_loop, we read here in the
                        // reverse order
...
int fiber_wait_for_event(int fd, uint32_t events) {
  ...
  // this code should really use ev_io_init(), but ev_io_init has compile
  // warnings.
...
static void timer_trigger(struct ev_loop* loop, ev_timer* watcher,
                          int revents) {
...
int fiber_sleep(uint32_t seconds, uint32_t useconds) {
  ...
  // this code should really use ev_timer_init(), but ev_timer_init has compile
  // warnings.
