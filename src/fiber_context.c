...
#define STACK_REGISTER(context, stack_location, stack_size)  \
  do {                                                       \
    (context)->ctx_stack_id = VALGRIND_STACK_REGISTER(       \
        stack_location, (char*)stack_location + stack_size); \
  } while (0)
...
#else
#define STACK_REGISTER(context, stackLoc, stackSize) \
  do {                                               \
  } while (0)
#define STACK_DEREGISTER(context) \
  do {                            \
...
extern void __splitstack_block_signals_context(splitstack_context_t context,
                                               int* new, int* old);
...
extern void* __splitstack_makecontext(size_t stack_size,
                                      splitstack_context_t context,
                                      size_t* size);
...
static size_t fiber_round_to_page_size(size_t size) {
  if (!fiberPageSize) {
    ...
    fiberPageSize -= 50;  // account for overhead for page info strucures (i
                          // don't know the actual size, this is a guess)
  }
...
static int fiber_context_alloc_stack(fiber_context_t* context,
                                     size_t stack_size) {
#if defined(FIBER_STACK_SPLIT)
  context->ctx_stack = __splitstack_makecontext(
      stack_size, context->splitstack_context, &context->ctx_stack_size);
  int off = 0;
  ...
  context->ctx_stack = mmap(0, context->ctx_stack_size, PROT_READ | PROT_WRITE,
...
int fiber_context_init(fiber_context_t* context, size_t stack_size,
  ...
  context->ctx_stack_pointer =
      (void**)((char*)context->ctx_stack + context->ctx_stack_size) - 1;
  context->ctx_stack_pointer = (void*)((uintptr_t)context->ctx_stack_pointer &
                                       ~0x0f);  // 16 byte stack alignment
  --context->ctx_stack_pointer;  // ctx_stack_pointer must be decremented
                                 // a-multiple-of-4 times to maintain 16 byte
                                 // alignement. this decrement is a dummy/filler
                                 // decrement
  ...
  *--context->ctx_stack_pointer = (void*)run_function;
  ...
  assert(((uintptr_t)context->ctx_stack_pointer & 0x0f) ==
         0);  // verify 16 byte alignment
...
void fiber_context_swap(fiber_context_t* from_context,
  ...
  /*
      ...
      For the current fiber:
          1. ebp and the new program counter are saved to the stack (the new
     program counter will be used when the fiber resumes)
          2. the current fiber's stack pointer is saved into the 'from' context
      For the fiber to be resumed:
          1. the stack pointer is retrieved from the 'to' context. the stack is
     now switched.
          2. the new program counter is popped off the stack and jumped to. the
     'to' context is now executing
          3. ebp is restored
      ...
      Credits:
          1. the original context switch was taken from Boost Coroutine, written
     as a Google Summer of Code project, Copyright 2006 Giovanni P. Deretta
             (http://www.crystalclearsoftware.com/soc/coroutine/). This work is
     subject to the Boost Software Licence Version 1.0 as copied below.
          2. Michael Ploujnikov for reducing the amount of context saved to the
     stack
  ...
  */
  ...
  __asm__ volatile(
      ...
      "\n\t movl %[to], %%esp"
      "\n\t popl %%ecx"
      "\n\t jmp  *%%ecx"  // Future Optimization: if ecx is label '0', no need
                          // for the jmp (ie. if the fiber is alread started)
      "\n0:\t popl %%ebp"
      :
      : [from] "a"(from_sp), [to] "d"(to_sp)
      : "cc", "%ecx",
#ifndef SHARED_LIB
        "%ebx",
#endif
        "%edi", "%esi", "%st", "%st(1)", "%st(2)", "%st(3)", "%st(4)", "%st(5)",
        "%st(6)", "%st(7)", "memory");
}
...
int fiber_context_init(fiber_context_t* context, size_t stack_size,
  ...
  context->ctx_stack_pointer = (void*)((uintptr_t)context->ctx_stack_pointer &
                                       ~0x0f);  // 16 byte stack alignment
  --context
        ->ctx_stack_pointer;  // ctx_stack_pointer must be decremented an even
                              // number of times to maintain 16 byte alignement.
                              // this decrement is a dummy/filler decrement
  *--context->ctx_stack_pointer = param;
  *--context->ctx_stack_pointer = NULL; /*dummy return address*/
  *--context->ctx_stack_pointer = (void*)run_function;
...
void fiber_context_swap(fiber_context_t* from_context,
  ...
  /*
      ...
      A fiber's stack looks like this while it's not running (stop grows down
     here to match memory): param (new contexts only) return address (dummy, new
     contexts only) rip (after resuming) rbp rbx r12 r13 r14 r15

      For the current fiber:
          1. calculate the rip for the resume (leaq 0f(%%rip), %%rax -> load
     address of the label '0' into rax)
          2. save new rip (rax), rbp, rbx, r12, r13, r14, r15
          3. the current fiber's stack pointer is saved into the 'from' context
      For the fiber to be resumed:
          1. rip is fetched off the stack early (so it's available when it's
     time to jump)
          2. the stack pointer is retrieved from the 'to' context. the stack is
     now switched.
          3. restore r15, r14, r13, r12, rbx, rbp for the 'to' fiber
          4. load 'param' into rdi (needed for new contexts). note that rdi is
     the first parameter to a function
          5. correct the stack pointer (ie. counter the 'push rax', without
     overwriting rip)
      ...
      Credits:
          1. a good chunk of the context switch logic/algorithm was taken from
     Boost Coroutine, written as a Google Summer of Code project, Copyright 2006
     Giovanni P. Deretta (http://www.crystalclearsoftware.com/soc/coroutine/).
     This work is subject to the Boost Software Licence Version 1.0 as copied
     below.
          2. the boost coroutine switch does not write rip as far as I can tell
     - I added the leaq 0f(%%rip), %%rax / push rax logic (Brian Watling)
          3. the boost coroutine switch does not save r12 through r15, and
     needlessly saves rax and rdx - i corrected this (Brian Watling)
  ...
  */
  ...
  __asm__ volatile(
      ...
      "popq  %%rbx\n\t"
      "popq  %%rbp\n\t"
      "movq  64(%[to]), %%rdi\n\t"  // Future Optimization: no need to load this
                                    // if the fiber has already been started
      "add   $8, %%rsp\n\t"
      "jmp   *%%rcx\n\t"  // Future Optimization: if rcx is label '0', no need
                          // for the jmp (ie. if the fiber is alread started)
      "0:\n\t"
      :
      : [from] "D"(from_sp), [to] "S"(to_sp)
      : "cc", "memory");
}
...
void fiber_context_swap(fiber_context_t* from_context,
  ...
  __splitstack_setcontext(to_context->splitstack_context);
#endif
  swapcontext((ucontext_t*)from_context->ctx_stack_pointer,
              (ucontext_t*)to_context->ctx_stack_pointer);
}
